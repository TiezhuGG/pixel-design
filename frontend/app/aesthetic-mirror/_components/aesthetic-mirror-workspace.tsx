...
export default function AestheticMirrorWorkspace({
  ...
  useEffect(() => {
    ...
    const loadModels = async () => {
      try {
        ...
        const availableModelIds = new Set((payload.imageModels || []).map((item) => item.runtimeId))
        const resolveImageModelId = (current: string) => {
          const value = String(current || "").trim()
          return value && availableModelIds.has(value) ? value : payload.imageModel?.runtimeId || ""
        }
        setWorkspaces((previous) => ({
          single: {
            ...previous.single,
            settings: {
              ...
              imageModelId: resolveImageModelId(previous.single.settings.imageModelId),
            },
          },
          batch: {
            ...previous.batch,
            settings: {
              ...
              imageModelId: resolveImageModelId(previous.batch.settings.imageModelId),
            },
          },
          sku: {
            ...previous.sku,
            settings: {
              ...
              imageModelId: resolveImageModelId(previous.sku.settings.imageModelId),
            },
          },
        }))
      } catch (error) {
  ...
  const syncJobSnapshot = async (jobId: string, signal: AbortSignal) => {
    const response = await fetch(`/api/aesthetic-mirror/jobs/${encodeURIComponent(jobId)}/sync`, {
      method: "POST",
      cache: "no-store",
      signal,
      credentials: "include",
  ...
  const pollJobUntilFinished = async (jobId: string, signal: AbortSignal) => {
    ...
    while (true) {
      ...
      if (pollCount >= 5 && pollCount % 3 === 2) {
        const syncedJob = await syncJobSnapshot(jobId, signal)
