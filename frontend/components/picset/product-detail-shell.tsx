...
function ProductDetailHeader() {
  ...
  const [siteName, setSiteName] = useState("PixelDesign")

  useEffect(() => {
    let cancelled = false
    fetchCachedPublicSystemSettings()
      .then((settings) => {
        if (!cancelled) setSiteName(normalizePublicAppName(settings.appName, "PixelDesign"))
      })
      .catch(() => {})
    return () => {
      cancelled = true
...
const PROMO_BANNER_DISMISSED_KEY = "PixelDesign:promo-banner:nova-img-2:dismissed"
