...
export class OpenAICompatibleProvider {
  ...
  async chat(messages: unknown[], options: Record<string, unknown>) {
    const baseUrl = normalizeEndpointBase(String(this.config.baseUrl || ""), "https://api.openai.com/v1")
  ...
  async editImage(imageUrls: string[], prompt: string, modelId?: string, options: Record<string, unknown> = {}) {
    ...
    const endpoint = buildImageGenerationEndpoint(baseUrl, "https://api.openai.com/v1")
    const response = await fetch(endpoint, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
