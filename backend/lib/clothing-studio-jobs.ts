      ...
      onTaskSubmitted: async (task) => {
        updateJob<"IMAGE_GEN">(params.jobId, (current) => ({
          ...
          provider_meta: {
            ...
            task_id: String(task.taskId || "").trim(),
            endpoint_base: String(task.endpointBase || "").trim(),
            model: deriveProviderMetaModel(String(task.modelId || current.payload.model || "").trim()),
            source: String(task.provider || current.provider_meta?.source || "").trim(),
          },
          result_data: current.result_data
            ? {
                ...current.result_data,
                task_id: String(task.taskId || current.result_data.task_id || "").trim(),
              }
            : current.result_data,
        }))
      },
...
export async function completeClothingStudioImageProviderTask(params: {
  jobId: string
  resultUrl: string
  modelId: string
  provider: string
  ...
  const completedAt = nowIso()
  const updated = updateJob<"IMAGE_GEN">(params.jobId, (current) => ({
    ...current,
    status: "success",
    updated_at: completedAt,
    ...
    provider_meta: {
      ...(current.provider_meta || {}),
      model: deriveProviderMetaModel(params.modelId || String(current.provider_meta?.model || current.payload.model || "")),
      source: String(params.provider || current.provider_meta?.source || "").trim(),
      provider_chain_tried: [String(params.provider || current.provider_meta?.source || "").trim()].filter(Boolean),
    },
  }))
  return updated
}
