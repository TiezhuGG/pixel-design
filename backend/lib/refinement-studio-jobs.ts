      ...
      onTaskSubmitted: async (task) => {
        await updateJob<"REFINE_IMAGE_GEN">(params.jobId, (current) => ({
          ...
          provider_meta: {
            ...
            model: String(task.modelId || current.provider_meta?.model || current.payload.model || "").trim(),
            source: String(task.provider || current.provider_meta?.source || "").trim(),
          },
          result_data: current.result_data
            ? {
                ...current.result_data,
                task_id: String(task.taskId || (current.result_data as { task_id?: unknown }).task_id || "").trim(),
              }
            : current.result_data,
        }))
      },
...
export async function completeRefinementStudioImageProviderTask(params: {
  jobId: string
  resultUrl: string
  modelId: string
  provider: string
  ...
  const completedAt = nowIso()
  return await updateJob<"REFINE_IMAGE_GEN">(params.jobId, (current) => ({
    ...current,
    status: "success",
    updated_at: completedAt,
    ...
    result_data: {
      ...
      status: 2,
      message: "",
      task_id: String(current.provider_meta?.task_id || `image_${params.jobId}`).trim(),
    },
    provider_meta: {
      ...(current.provider_meta || {}),
