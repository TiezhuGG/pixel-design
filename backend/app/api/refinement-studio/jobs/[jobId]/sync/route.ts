...
import { completeRefinementStudioImageProviderTask, getRefinementStudioJob } from "@/lib/refinement-studio-jobs"
...
export async function POST(
  ...
  try {
    ...
    const job = await getRefinementStudioJob(jobId, scope.userId)
    if (!job) {
      return NextResponse.json({ error: "浠诲姟涓嶅瓨鍦ㄦ垨鏃犳潈璁块棶" }, { status: 404 })
    }
    if (job.type !== "REFINE_IMAGE_GEN" || job.status !== "processing") {
    ...
    const updated = await completeRefinementStudioImageProviderTask({
      jobId: job.id,
      resultUrl: imageUrl,
    ...
    console.error("[api/refinement-studio/jobs/:jobId/sync] failed:", error)
