...
async function resolvePendingImageTaskResult(result: any, apiKey: string) {
  if (!result?.pendingTask || !result?.taskId || !apiKey) return result
  const provider = String(result.provider || "").trim().toLowerCase()
  const endpointBase = String(result.endpointBase || "").trim()
  const taskId = String(result.taskId || "").trim()
  const imageUrl =
    provider.includes("modelscope") || endpointBase.toLowerCase().includes("api-inference.modelscope.")
      ? await pollModelScopeImageTask({ endpointBase, apiKey, taskId })
      : await pollApimartImageTask({ endpointBase, apiKey, taskId })
  return {
    ...result,
    imageUrl,
    pendingTask: false,
...
async function generateElementPng(params: {
  ...
  const credentials = resolveProviderCredentials(modelConfig?.provider || {})
  const apiKey = String(credentials.apiKey || modelConfig?.provider?.apiKey || modelConfig?.provider?.api_key || "").trim()
  ...
  try {
    const result = await withTimeout(
      ...
      }).then((generationResult) => resolvePendingImageTaskResult(generationResult, apiKey)),
      Number(process.env.CANVAS_SMART_LAYER_ELEMENT_TIMEOUT_MS || 180000),
...
function buildPhotoshopJsx(manifest: SmartLayerManifest, layerFiles: Array<{ layer: SmartLayer; relativePath: string }>) {
  const lines = [
    ...
    `var doc = app.documents.add(${manifest.canvas.width}, ${manifest.canvas.height}, 72, "PixelDesign Smart Layer", NewDocumentMode.RGB, DocumentFill.TRANSPARENT);`,
...
async function runSmartLayerJob(params: {
  ...
  const jobId = params.jobId
    ...
    await writeFile(
      ...
      [
        "PixelDesign Smart Layer Editable PSD Package",
        "",
        "1. Open Photoshop.",
