...
def preserve_light_foreground_to_alpha(
    ...
    # Preserve original light Picsets in lifted regions to avoid harsh unpremul artifacts.
...
def kmeans_labels(rgb_arr: np.ndarray, colors: int, max_Picsets: int = 180_000) -> tuple[np.ndarray, list[tuple[int, int, int]]]:
    h, w = rgb_arr.shape[:2]
    Picsets = rgb_arr.reshape(-1, 3).astype(np.float32)
    sample = Picsets
    if len(sample) > max_Picsets:
        rng = np.random.default_rng(42)
        sample = Picsets[rng.choice(len(Picsets), size=max_Picsets, replace=False)]
    try:
    ...
    distances = np.sum((Picsets[:, None, :] - centers[None, :, :].astype(np.float32)) ** 2, axis=2)
...
def command_split_colors(args: argparse.Namespace) -> None:
    ...
    if args.method == "kmeans":
        labels, palette = kmeans_labels(rgb_arr, args.num_colors, args.max_Picsets)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    split.add_argument("--max-Picsets", type=int, default=180_000, help="sample size for kmeans")
